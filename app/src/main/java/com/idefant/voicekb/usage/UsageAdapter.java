package com.idefant.voicekb.usage;
...
public class UsageAdapter extends RecyclerView.Adapter<UsageAdapter.RecyclerViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RecyclerViewHolder holder, final int position) {
        ...
        switch ((int) model.getModelProvider()) {
            case 0:
                modelProvider = activity.getString(R.string.voicekb_usage_model_provider_openai); break;
            case 1:
                modelProvider = activity.getString(R.string.voicekb_usage_model_provider_groq); break;
            case 2:
                modelProvider = activity.getString(R.string.voicekb_usage_model_provider_custom); break;
            default:
                break;
        }

        holder.itemModelNameTv.setText(VoiceKBUtils.translateModelName(model.getModelName()) + " (" + modelProvider + ")");
        holder.itemTotalCostValueTv.setText(activity.getString(R.string.voicekb_usage_cost, db.getCost(model.getModelName())));
        if (model.getInputTokens() == 0) {
            ...
            holder.itemAudioTimeValueTv.setText(activity.getString(R.string.voicekb_usage_audio_time, model.getAudioTime() / 60, model.getAudioTime() % 60));
        } else {
