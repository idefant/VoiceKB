...
Стабильность определяется по имени тега: суффикс через дефис (`v0.2.0-beta.1`) — предрелиз, без суффикса (`v0.2.0`) — стабильный. Предрелиз публикуется с пометкой «Pre-release» и не становится «Latest»; отметку «Latest» получают только стабильные теги.
