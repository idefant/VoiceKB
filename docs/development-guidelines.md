...
Релизы GitHub создаются пушем тега, начинающегося с `v` (например, `v0.2.0-beta.1`, `v0.2.0`). В `CHANGELOG.md` должен быть заголовок, совпадающий с именем тега (`## v0.2.0`) — по нему workflow находит нужную секцию.
