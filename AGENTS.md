...
На любое действие необходима явная команда пользователя. Не выдумывать и не додумывать команды к действию за пользователя. Предложение или идея, высказанные в обсуждении, — это не команда к выполнению: пока пользователь явно не сказал «делай», ничего не менять. Если объём или отдельный шаг не был явно одобрен — сначала спросить, потом выполнять.
...
- История распознаваний (список, прослушивание, копирование, повтор обработки, удаление, хранение аудио): `docs/functional-spec/history.md` + `docs/development/history-storage.md`.
